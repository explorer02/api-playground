...
//context
import { useTabState, MutationExecutorTabState } from '@/context/TabStateContext';
...
export const useMutationExecutor = ({ config, tabId, onExecutionComplete }: Params): ReturnType => {
  ...
  const saveField = useCallback(
    (field: keyof Pick<MutationExecutorTabState, 'input' | 'variables' | 'output'>, value: string) => {
      const current = getState<MutationExecutorTabState>(tabId);
      setState<MutationExecutorTabState>(tabId, {
  ...
  const onInputMount = useCallback<OnMount>(
    (editor, monaco) => {
      ...
      const saved = getState<MutationExecutorTabState>(tabId);
  ...
  const onSubmit = useCallback(async () => {
    ...
    try {
      ...
      setStats(newStats);
      const current = getState<MutationExecutorTabState>(tabId);
      setState<MutationExecutorTabState>(tabId, { ...current!, stats: newStats });
